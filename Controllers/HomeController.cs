...
namespace EconomizaAlagoasWeb.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(SefazApiClient sefazApiClient, ProdutoRepository produtoRepository)
        {
        ...
        public IActionResult Index()
        {
            ...
            {
                Municipios = ObterMunicipiosDeAlagoas()
            };
            return View(viewModel);
        }

        [HttpPost]
        public async Task<IActionResult> Pesquisar(PesquisaViewModel model)
        {
            model.Municipios = ObterMunicipiosDeAlagoas();
            ...
            try
            {
                var response = await _sefazApiClient.PesquisarProdutosAsync(model);
                var registros = response?.Conteudo ?? new List<Registro>();

                if (registros.Any())
                {
                    await _produtoRepository.SalvarDadosNoBanco(registros, DateTime.UtcNow, "Pesquisa Personalizada");
                }

                return View("Resultados", registros);
            }
            catch (HttpRequestException ex)
            {
                ModelState.AddModelError(string.Empty, $"Erro ao se comunicar com a API: {ex.Message}. Verifique o console para mais detalhes.");
                return View("Index", model);
            }
            catch (Exception ex)
            {
        ...
        private List<SelectListItem> ObterMunicipiosDeAlagoas()
        {
            var municipios = new Dictionary<int, string>
            {
                ...
                { 2707008, "PINDOBA" }, { 270107, "PIRANHAS" }, { 2707206, "POCO DAS TRINCHEIRAS" },
